...
export class BurhanPoseEditor {
  ...
  async captureViewportFrame(requestedWidth = 1560, requestedHeight = 1400) {
    if (this.exporting) throw new Error("An export is already in progress.");
    ...
    const desiredWidth = Math.max(512, Math.round(Number(requestedWidth) || 1560));
    const desiredHeight = Math.max(512, Math.round(Number(requestedHeight) || 1400));
    const outputScale = Math.min(1, maxResolution / Math.max(desiredWidth, desiredHeight));
    const width = Math.round(desiredWidth * outputScale);
    const height = Math.round(desiredHeight * outputScale);
    ...
    if (captureCamera.isPerspectiveCamera) {
      captureCamera.aspect = aspect;
    } else {
      const halfHeight = this.orthographicHeight / 2;
      captureCamera.left = -halfHeight * aspect;
      captureCamera.right = halfHeight * aspect;
    ...
    try {
      this.avatars.forEach((avatar) => avatar.character?.traverse((object) => {
        ...
        materials.forEach((material) => {
          ...
          highlights.push([material, material.emissiveIntensity]);
      ...
      const blob = await new Promise((resolve) => this.renderer.domElement.toBlob(resolve, "image/png"));
      if (!blob) throw new Error("The browser could not capture the center view.");
      return blob;
    } finally {
